		...
		if (product != null && product.isStocked())
		{
			...
			else
			{
				...
				if (!MStorageOnHand.add(docLine.getCtx(), M_Warehouse_ID, 
					docLine.getM_Locator_ID(),
					docLine.getM_Product_ID(), 
					docLine.getM_AttributeSetInstance_ID(), Qty, docLine.get_TrxName()))
				{
					throw new AdempiereException(); //Cannot correct Inventory (MA)
				}
			...
			int M_Warehouse_ID)
	{
		...
		if(line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			if (incomingTrx)
			{
				...
				MStorageOnHand[] storages = MStorageOnHand.getWarehouse(line.getCtx(), M_Warehouse_ID, line.getM_Product_ID(), 0,
			...
			else
			{
				...
				MStorageOnHand[] storages = MStorageOnHand.getWarehouse(line.getCtx(), M_Warehouse_ID, line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(), 
