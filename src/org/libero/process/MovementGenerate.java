...
public class MovementGenerate extends SvrProcess
{
			...
			while (rs.next ())		//	Order
			{
				...
				for (int i = 0; i < lines.length; i++)
				{
					...
					if (p_IsUnconfirmedInOut && product != null && toDeliver.signum() != 0)
					{
						...
						LiberoMovementLine[] iols = LiberoMovementLine.getOfOrderLine(getCtx(), 
					...
					//
					MStorageOnHand[] storages = getStorages(l.getM_Warehouse_ID(), 
		...
		MStorageOnHand[] storages, boolean force)
	{
		...
		if (storages == null)
		{
			LiberoMovementLine line = new LiberoMovementLine (m_movement);
		...
		for (int i = 0; i < storages.length; i++)
		{
			...
			if (!linePerASI)	//	find line with Locator
			{
				...
				{
					LiberoMovementLine test = (LiberoMovementLine)list.get(ll);
			...
			{
				line = new LiberoMovementLine (m_movement);
