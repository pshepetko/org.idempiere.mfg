...
//		BigDecimal sumQtyOrdered = BigDecimal.ZERO;
		...
		for(int i = 0; i < ids.length; i++) {
			...
			BigDecimal available = (MStorageReservation.getQtyAvailable( storage.getM_Warehouse_ID(),p.get_ID(), asi.get_ID(), null));		
...
//			sumQtyOrdered = sumQtyOrdered.add(storage.getQtyOrdered());
	    	...
	    	obj = new Object[] {
					...
					storage.getQtyOnHand().subtract(sumQtyAvailable), //red1 QtyReserved		
//					storage.getQtyOrdered(),
		...
		if(count > 1) {
			...
			obj = new Object[] {
					...
					storage.getQtyOnHand().subtract(sumQtyAvailable), //red1 QtyReserved	
//					sumQtyOrdered,
					sumQtyAvailable //red1 sumQtyOnHand.subtract(sumQtyReserved)
	    	};
	    	
			sb.append(MessageFormat.format(STORAGE_SUM_LINE_INFO_PATTERN, obj));
		}

		double available = sumQtyAvailable.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
