...
/**
 * adapted from MRP.java that bulk process all MRP records.
 * this process give option of selected records from InfoWindow - MRP Info (ZK)
 * InfoWindow adapted from VMRPDetailed.java, thus smaller footprint easier maintenance.
 * @author red1
 *
 */
public class CalculateMaterialPlan extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		for (MPPMRP mrp:mrpset){
			//DELETE if status is closed
			if (p_DeleteMRP){
				deleteMRP(mrp);
			}
	 		//DO NOT PROCESS if it is Demand & Forecast & DatePromised before today
			if (MPPMRP.TYPEMRP_Demand.equals(mrp.getTypeMRP())
					&& MPPMRP.ORDERTYPE_Forecast.equals(mrp.getOrderType())
			...
			final Timestamp DateStartSchedule = mrp.getDateStartSchedule();
			...
			if (S_Resource_ID==0){
				log.severe("NO PLANT / RESOURCE FOR "+mrp.getM_Product());
				continue;
			}
			//
			//Set Resource Plant to get Planning Horizon (based on MRP.java whole process)
			MResource plant = new Query(Env.getCtx(), MResource.Table_Name, MResource.COLUMNNAME_S_Resource_ID
					+"=?", get_TrxName())
			...
			if (product == null || product.get_ID() != mrp.getM_Product_ID())
			{
				...
				if (QtyGrossReqs.signum() != 0)
				{
							...
							&& POQDateStartSchedule.compareTo(Planning_Horizon) < 0) 
					{
						...
						calculatePlan(mrp,AD_Client_ID, AD_Org_ID,M_Warehouse_ID ,BeforePP_MRP_ID , product ,BeforeDateStartSchedule);
					}
					...
					{
						// TODO: Q: when we have this situation because on LFL we balance the Demand immediately
						//		so we do not accumulate it?
						calculatePlan(mrp,AD_Client_ID, AD_Org_ID,M_Warehouse_ID ,BeforePP_MRP_ID , product ,BeforeDateStartSchedule );
					}
				...
				log.info("Calculate Plan for this Product:" + product);
			...
			{
				calculatePlan(mrp,AD_Client_ID,AD_Org_ID,M_Warehouse_ID,PP_MRP_ID,product ,DatePromisedFrom);						
			...
			// If  Order_Policy = LotForLot then always create new range for next period and add QtyGrossReqs          
			else if (X_PP_Product_Planning.ORDER_POLICY_Lot_For_Lot.equals(m_product_planning.getOrder_Policy()))
			{                                                                                                                                           
				QtyGrossReqs = QtyGrossReqs.add(Qty);
				BeforeDateStartSchedule = DatePromised; 		
				calculatePlan(mrp,AD_Client_ID, AD_Org_ID,M_Warehouse_ID,PP_MRP_ID,product,BeforeDateStartSchedule); 		
				continue;
			}                                                                        
		} // end while
		...
		{   
			...
			{
				BeforeDateStartSchedule =  POQDateStartSchedule; 
				calculatePlan(null,AD_Client_ID,AD_Org_ID,M_Warehouse_ID,BeforePP_MRP_ID , product ,BeforeDateStartSchedule);
			}
			else if (X_PP_Product_Planning.ORDER_POLICY_Lot_For_Lot.equals(m_product_planning.getOrder_Policy())
					&& BeforeDateStartSchedule.compareTo(Planning_Horizon) <= 0 )
			{
		...
		//red1 put the counts into the endResult pop up string
		if (endResult.length()<1){
			endResult.append("Total MO:  "+count_MO );
			endResult.append(" Total Req:  "+count_MR+MPPOrder.count_MR);
			endResult.append(" Total DO:  "+count_DO);
			endResult.append(" Total Msg: "+count_Msg);
		}
		return endResult.toString() ;
	}
	
	/**************************************************************************
	 * 	Calculate Plan for this product
								...
								MProduct product, Timestamp DemandDateStartSchedule) throws SQLException
	{
		...
		if (QtyPlanned.signum() > 0)    
		{
			...
			for (int ofq = 1 ; ofq <= loops ; ofq ++ )
			{
				...
				try
				{
					createSupply(mrp,AD_Org_ID, PP_MRP_ID, product, QtyPlanned, DemandDateStartSchedule);
				}
				catch (Exception e)
				{
	 ...
	 * @throws SQLException 
	 */
	protected void createSupply(MPPMRP mrp,int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal QtyPlanned ,Timestamp DemandDateStartSchedule)
	throws AdempiereException, SQLException
	{
		...
		{
			createRequisition(PP_MRP_ID,AD_Org_ID, product, QtyPlanned ,DemandDateStartSchedule);
		}
		// Manufacturing Order
		else if (product.isBOM())
		{
			createPPOrder(mrp,AD_Org_ID, PP_MRP_ID, product,QtyPlanned, DemandDateStartSchedule);
		}
		else
		{
	...
	protected void createRequisition(int PP_MRP_ID, int AD_Org_ID, MProduct product, BigDecimal QtyPlanned, Timestamp DemandDateStartSchedule)
	throws AdempiereException, SQLException
	{
 		if (duration==0) //do only once
			duration = MPPMRP.getDurationDays(null,QtyPlanned, m_product_planning);
		Timestamp DateRequired = TimeUtil.addDays(DemandDateStartSchedule, 0 - duration);
		
		log.info("Create Requisition");
		MRequisition req = new  MRequisition(getCtx(),0, get_TrxName());  		
		req.create(PP_MRP_ID, QtyPlanned, m_product_planning.getM_Product_ID(),m_product_planning.getC_BPartner_ID(), AD_Org_ID, m_product_planning.getPlanner_ID(), DateRequired, "Generate from MRP", m_product_planning.getM_Warehouse_ID(), docTypeReq_ID);

		count_MR += 1;
	}
	
	protected void createPPOrder(MPPMRP mrp,int AD_Org_ID, int PP_MRP_ID, MProduct product,BigDecimal QtyPlanned,Timestamp DemandDateStartSchedule)
	throws AdempiereException, SQLException
	{		
		...
		MPPOrder order;
		MPPOrder check = new Query(Env.getCtx(),MPPOrder.Table_Name,MPPOrder.COLUMNNAME_PP_Order_ID+"=?",mrp.get_TrxName())
		.setParameters(mrp.getPP_Order_ID())
		.first();
		if (check==null){	
			order = new MPPOrder(getCtx(), 0, get_TrxName());		
			order.addDescription("Generated by MRP");
			order.setAD_Org_ID(AD_Org_ID);
			order.setLine(10);
			log.info("PP Order Created");
		}			
		else {
			order = check;
		} 
		...
		{
			log.info("Maintenance Order BOMTypeMaintenance");//DocBaseType = MOF
			MDocType dt = new Query(Env.getCtx(),MDocType.Table_Name,MDocType.COLUMNNAME_DocBaseType+"=?",get_TrxName())
			.setParameters("MOF")
			.first();
			docTypeMF_ID = dt.getC_DocType_ID();
			order.setC_DocTypeTarget_ID(docTypeMF_ID);
			order.setC_DocType_ID(docTypeMF_ID); 
		}
		else
		{	
			log.info("Manufacturing Order BOMType"); //DocBaseType = MOP
			MDocType dt = new Query(Env.getCtx(),MDocType.Table_Name,MDocType.COLUMNNAME_DocBaseType+"=?",get_TrxName())
			.setParameters("MOP")
			.first();
			docTypeMO_ID = dt.getC_DocType_ID();
		...
		order.setDatePromised(DemandDateStartSchedule);
		if (duration==0) //do only once
			 duration = MPPMRP.getDurationDays(mrp, QtyPlanned, m_product_planning);

		order.setDateStartSchedule(TimeUtil.addMinutess(DemandDateStartSchedule, 0 - duration)); //red1 add Days was wrong as duration is mins
	...
	protected MPPProductPlanning getProductPlanning(int AD_Client_ID , int AD_Org_ID, int S_Resource_ID , int M_Warehouse_ID, MProduct product) throws SQLException
	{
		...
		MPPProductPlanning pp2 = new MPPProductPlanning(getCtx(), 0 , get_TrxName());                                                       
...
//		 + ":" + DemandDateStartSchedule;
	...
	protected void deleteMRP(MPPMRP mrp) throws SQLException
	{
		...
		String sql = "DELETE FROM PP_MRP WHERE OrderType = 'MOP' AND DocStatus ='CL' AND AD_Client_ID=" + mrp.getAD_Client_ID()  + " AND AD_Org_ID=" + mrp.getAD_Org_ID() + " AND M_Warehouse_ID="
		+mrp.getM_Warehouse_ID() +  " AND S_Resource_ID="+mrp.getS_Resource_ID() ;					
