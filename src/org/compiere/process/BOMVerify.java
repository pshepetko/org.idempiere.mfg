 ...
 *	
 *  @author Jorg Janke
 *  @author Peter Shepetko 03.09.2019
...
public class BOMVerify extends SvrProcess
{
	...
	private boolean validateProduct (MProduct product)
	{
		...
		Timestamp sysdate = Env.getContextAsDate(getCtx(), "#Date");
		...
		//Issue #17 Verify BOM Structure without Components
		List<MPPProductBOMLine> productsBOMLines = new Query(product.getCtx(), MPPProductBOMLine.Table_Name, 
						MPPProductBOMLine.COLUMNNAME_PP_Product_BOM_ID+"=?",
						product.get_TrxName())
						.setParameters(MPPProductBOM.getBOMSearchKey(product))
						.list();
		...
		for (MPPProductBOMLine productsBOMLine : productsBOMLines)
		{
			if (! productsBOMLine.isActive())
				continue;
			lines++;
			MProduct pp = new MProduct(getCtx(), productsBOMLine.getM_Product_ID(), get_TrxName());
			if (!pp.isBOM()) {
